...
public class FilterClient {
        ...
        for (Phone phone : allPhones) {
            ...
            switch (value) {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    if (!aviable.contains(phone.getMemory())) {
                        aviable.add(phone.getMemory());
                    }
                    break;
                case 6:
                    if (!aviable.contains(phone.getStorage())) {
                        aviable.add(phone.getStorage());
                    }
                    break;
                case 7:
                    if (!aviable.contains(phone.getPrice())) {
                        aviable.add(phone.getPrice());
                    }
                    break;
    ...
    public HashMap<Filter, Object> serchFiltering(List<Phone> allPhones) {
        ...
        while (ok == 0 || ok == 1) {
            ...
            System.out.println("Choose the number which matches the filter you are looking for" +
                    "1 Brand   2 Color  3 Phone Type  4 Model  5 RAM Memory 6 Storage 7 Price");
                ...
                if (chosenValue == 0)
                    ...
                    if (chosenOption <= i) {
                        if (chosenValue == 1)
                            ...
                            filtersList.put(Filter.MODEL, options.get(chosenOption - 1).toString());
                        else if (chosenValue == 5)
                            filtersList.put(Filter.RAM, options.get(chosenOption - 1).toString());
                        else if (chosenValue == 6)
                            filtersList.put(Filter.STORAGE, options.get(chosenOption - 1).toString());
                        else if (chosenValue == 7)
                            filtersList.put(Filter.PRICE, options.get(chosenOption - 1).toString());
    ...
    public void applyFilterList(HashMap<Filter, Object> filterList, List<Phone> allPhones) {
        ...
        while (it.hasNext()) {
            Map.Entry<Filter, Object> pair = (Map.Entry<Filter, Object>) it.next();
                ...
                if (stop != 0) {
                    ...
                    switch (currentKey.toString()) {
                        ...
                        case "PRICE":
                            ...
                            PriceCriteria priceCriteria = new PriceCriteria(Integer.parseInt(String.valueOf(currentValue)));
                            appliedFilterList = ifNotExistDelete(appliedFilterList, priceCriteria.meetCriteria(allPhones), allPhones.get(j));
                            display(j, checkIfHasNext, filterList.size() - 1, appliedFilterList);
                        ...
                        case "STORAGE":
                            // currentValue=Integer.parseInt(currentValue);
                            StorageCriteria storageCriteria = new StorageCriteria(Integer.parseInt(String.valueOf(currentValue)));
                            appliedFilterList = ifNotExistDelete(appliedFilterList, storageCriteria.meetCriteria(allPhones), allPhones.get(j));
                            display(j, checkIfHasNext, filterList.size() - 1, appliedFilterList);
                        ...
                        case "RAM":
                            // currentValue=Integer.parseInt(currentValue);
                            RAMCriteria ramCriteria = new RAMCriteria(Integer.parseInt(String.valueOf(currentValue)));
                            appliedFilterList = ifNotExistDelete(appliedFilterList, ramCriteria.meetCriteria(allPhones), allPhones.get(j));
                            display(j, checkIfHasNext, filterList.size() - 1, appliedFilterList);
